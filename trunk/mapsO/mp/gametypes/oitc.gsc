		...
		if ( level.scr_oitc_suddendeath_timelimit > 0 ) {
			...
			if ( level.scr_oitc_suddendeath_show_enemies == 1 ) {
				for ( index = 0; index < level.players.size; index++ ) {
					level.players[index] thread maps\mp\gametypes\_hud_message::oldNotifyMessage( &"OW_SUDDEN_DEATH", &"OW_ONE_LIFE_UAV_ON", undefined, (1, 0, 0), "mp_last_stand" );
					level.players[index] setClientDvar( "g_compassShowEnemies", 1 );
					if( level.players[index].pers["lives"] > 0 )
						level.players[index].pers["lives"] = 0;
				}
			}
...
onSpawnPlayer()
{
	...
	if( level.OITCExtraTime ) {
		self thread maps\mp\gametypes\_hud_message::oldNotifyMessage( &"OW_SUDDEN_DEATH", &"OW_ONE_LIFE_UAV_ON", undefined, (1, 0, 0), "mp_last_stand" );
