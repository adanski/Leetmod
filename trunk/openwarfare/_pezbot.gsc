...
StartDev()
{
	...
	while(1) {
		...
		else if (autowp && (distance(level.playerPos, lastview) > 60 || dvar<60)) {
...
TestClient(team)
{
	...
	if(getDvar("scr_game_perks") == "1" )
		if(getDvar("scr_pezbots_useperks") == "1") {
			...
			switch(randomint(5)) {
				case 0:
					self setPerk("specialty_bulletaccuracy"); 		//Steady Aim
					break;
				case 1:
					self setPerk("specialty_armorvest"); 			//Juggernaut
					break;
				case 2:
					self setPerk("specialty_gpsjammer"); 			//UAV Jammer
					break;
				case 3:
					self setPerk("specialty_explosivedamage"); 		//Sonic Boom
					break;
				case 4:
...
{
	weaponCount = int(TableLookup("mp/PeZBOT_WeaponDefs.csv", 0, 0, 2));
	
	randomWeapon = 1 + randomintrange(0, weaponCount+1);
	
	self.actualWeapon = TableLookup("mp/PeZBOT_WeaponDefs.csv", 0, randomWeapon, 1);
	self.maxEngageRange = int(TableLookup("mp/PeZBOT_WeaponDefs.csv", 0, randomWeapon, 2));
}
...
TryPlayGame()
{
	...
	if(isDefined(self.bSuppressingFire) && self.bSuppressingFire && isDefined(self.leader) && isDefined(self.suppressTarget)) {
		...
		if(isDefined(self.bFollowTheLeader) && self.bFollowTheLeader && isDefined(self.leader)) {
			...
			else if(leaderDistance > (200*200)) {
			...
			if(isDefined(self.bHoldPosition) && self.bHoldPosition && isDefined(self.leader)) {
				...
				if(level.gametype == "koth") {
					...
					if(level.gametype == "sd") {
						...
						else if(level.gametype == "dm" || level.gametype == "war") {
							if(isDefined(self.bestTarget)) {
...
//                          Print3d(self.origin + (0,0,70), "dodging", (1,0,0), 4);
					...
					else if(isdefined(level.chopper) && !self CanSeeTarget() && self.hasChopperTarget) {
						self.bFaceNearestEnemy = true;
		...
		if(players.size > 0/* && (!level.gameEnded/* || !level.inPrematchPeriod)*/) {
			for(i = 0; i < players.size; i++) {
				...
				if(self IsEnemy(player) && isdefined(player.sessionstate) && player.sessionstate == "playing") {
					...
					else if((!self AttackedMe(player) && !self CanSee(player)) && isdefined(level.chopper) && self CanSee(level.chopper) && (level.chopper.currentstate != "leaving" || level.chopper.currentstate != "crashing")) {
			...
			if(self.bFaceNearestEnemy && isdefined(self.bestTarget) && !self IsStunned()/*  && (!level.gameEnded || !level.inPrematchPeriod)*/) {
				...
				else if(skill > 0 && self.bShooting) {
...
MonitorMovement()
{
	...
	while(1) {
		...
		if(1) {
			...
			else if(self.stance == "climb") {
			
			}
			
			//running
			else if(self.fBaseMoveSpeed > 8.5 && self.stance != "crouch") {
				if(isdefined(dist) && dist >= self.botRunDist) {
					newMoveType = "run";
					self.fMoveSpeed = 9.0;
				}
	...
	while(1) {
		if(CanDebugDraw() && isDefined(level.waypoints) && isDefined(level.waypointCount) && level.waypointCount > 0) {
			...
			for(i = 0; i < level.waypointCount; i++) {
				...
				else if(level.waypoints[i].type == "climb") { //yellow for climbing
					color = (1,1,0);
				}
...
//        print3d(level.waypoints[i].origin + (0,0,110), "Index: " + i, (1,1,1), 2);
			...
			if(isdefined(level.killpoints)) {
				for(i = 0; i < level.killpointCount; i++) {
					...
					else if(level.killpoints[i].childCount == 1) { //purple for dead ends
						color = (1,0,1);
					}
					else { //green for linked
						color = (0,1,0);
