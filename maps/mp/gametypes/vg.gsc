...
precacheVisuals()
{
	...
	switch( level.guide_color ) {
		case "red":
			color = game["circle_effect"]["red"];
			break;
		case "black":
			color = game["circle_effect"]["black"];
			break;
		case "gold":
			color = game["circle_effect"]["gold"];
			break;
		case "silver":
			color = game["circle_effect"]["silver"];
			break;
...
initGuide()
{
	...
	switch( level.vg_gametype ) {
		...
		default:
			assertMsg( "Gametype is not supported." );
			logPrint( "Visual Guide Error: Gametype Not Supported" );
			break;
	}
}
